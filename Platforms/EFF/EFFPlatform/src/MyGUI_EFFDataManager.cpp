...
namespace MyGUI
{
	...
	MyGUI_EFFDataManager::MyGUI_EFFDataManager() :
		mIsInitialise(false)
	{
	}

	void MyGUI_EFFDataManager::initialise()
	{
	...
	void MyGUI_EFFDataManager::shutdown()
	{
	...
	IDataStream* MyGUI_EFFDataManager::getData(const std::string& _name)
	{
	...
	void MyGUI_EFFDataManager::freeData(IDataStream* _data)
	{
		delete _data;
	}

	bool MyGUI_EFFDataManager::isDataExist(const std::string& _name)
	{
		const VectorString& files = getDataListNames(_name);
		return !files.empty();
	}

	const VectorString& MyGUI_EFFDataManager::getDataListNames(const std::string& _pattern)
	{
	...
	const std::string& MyGUI_EFFDataManager::getDataPath(const std::string& _name)
	{
	...
	void MyGUI_EFFDataManager::addResourceLocation(const std::string& _name, bool _recursive)
	{
		ArhivInfo info;
